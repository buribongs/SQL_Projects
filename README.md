...
As can be seen from the analysis, the data is incredibly messy. To clean this dataset, I will employ a sytematic ETL approach, cleaning column after column while leaving the original dataset unaltered. This approach will use rely heavily on SELECT statements, CASE logic, and built-in SQL functions (TRIM, REPLACE, CAST, date functions) to derive clean values.
